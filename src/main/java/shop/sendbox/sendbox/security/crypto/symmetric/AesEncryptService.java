...
public class AesEncryptService implements SymmetricCryptoService {
	...
	private SecretKey getSecretKey() throws Exception {
		KeySpec spec = new PBEKeySpec(securityConfig.getSymmetric().getPassword().toCharArray(),
			securityConfig.getSymmetric().getSalt().getBytes(), ITERATION_COUNT, KEY_LENGTH);
	...
	private IvParameterSpec generateIv() {
		// 고정된 IV 사용 (16바이트)
		byte[] iv = "SendboxFixedIv16".getBytes();
		return new IvParameterSpec(iv);
	}
}
